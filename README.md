...
Переменные окружения: DB_HOST(хост), DB_PASSWORD(пароль), SECRET_KEY(секретный ключ), DB_PORT(номер порта), DB_NAME(имя), DB_USER(пользователь), DEBUG(отладка), ALLOWED_HOSTS(список разрешенных сайтов). Это переменные в которых должны храниться данные для подачи запроса в базу данных и настройки работы приложения. Для работы программы необходимо создать файл ".env"  и просвоить переменным соответствующие значения:
```
DB_HOST = '...'
DB_PORT = '...'
DB_NAME = '...'
